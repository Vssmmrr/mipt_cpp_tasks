...
namespace geometry {
    ...
    double Distance(const Segment& lhs, const Segment& rhs);
...
int main() {
    ...
    std::cout << std::fixed << geometry::Distance(first, second);
...
namespace geometry {
    ...
    double Distance(const Segment& lhs, const Segment& rhs) {
        if (lhs.CrossSegment(rhs)) {
            return 0;
        }
        const Point& A = lhs.GetBegin();
        const Point& B = lhs.GetEnd();
        const Point& C = rhs.GetBegin();
        const Point& D = rhs.GetEnd();

        return Min<double>(Distance(A, rhs), Distance(B, rhs), Distance(C, lhs), Distance(D, lhs));
    }
}
