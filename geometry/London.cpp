...
namespace geometry {
    ...
    std::vector<ssize_t> Zones(const std::vector<Point>& points);
}
...
int main() {
    ...
    std::vector<ssize_t> zones = geometry::Zones(point_set);
...
namespace geometry {
    ...
    bool OnOneLine(const Point& first, const Point& second, const Point& third) {
        return OrientedArea(first, second, third) == 0;
    }

    std::vector<size_t> BorderIndices(const std::vector<Point>& points,
                                      const std::vector<size_t>& order_indices) {
        size_t left = order_indices[0];
        size_t right = order_indices.back();
        std::vector<size_t> up, down;
        up.push_back(left);
        down.push_back(left);
        bool on_one_line = true;

        for (size_t i = 1; i < order_indices.size(); ++i) {
            if (!OnOneLine(points[left], points[order_indices[i]], points[right])) {
                on_one_line = false;
            }
            if (i == order_indices.size() - 1 ||
                LeftRotation(points[left], points[order_indices[i]], points[right])) {
                while (up.size() >= 2 &&
                       !LeftRotation(points[up[up.size() - 2]],
                                     points[up[up.size() - 1]],
                                     points[order_indices[i]])) {
                    up.pop_back();
                }
                up.push_back(order_indices[i]);
            }
            if (i == order_indices.size() - 1 ||
                RightRotation(points[left], points[order_indices[i]], points[right])) {
                while (down.size() >= 2 &&
                       !RightRotation(points[down[down.size() - 2]],
                                      points[down[down.size() - 1]],
                                      points[order_indices[i]])) {
                    down.pop_back();
                }
                down.push_back(order_indices[i]);
            }
        }
        ...
        std::vector<size_t> border_indices = up;
        for (size_t i = down.size() - 2; i > 0; --i) {
            border_indices.push_back(down[i]);
        }
        return border_indices;
    }

    std::vector<ssize_t> Zones(const std::vector<Point>& points) {
        std::vector<size_t> order_indices(points.size());
        for (size_t i = 0; i < points.size(); ++i) {
            order_indices[i] = i;
        }
        std::sort(order_indices.begin(), order_indices.end(),
                  [points](size_t lhs, size_t rhs) {
                      return Compare(points[lhs], points[rhs]);
                  });

        std::vector<ssize_t> zones(points.size(), -1);
        ssize_t zone = 0;
        while (order_indices.size() > 2) {
            std::vector<size_t> border_ind = BorderIndices(points, order_indices);
            if (border_ind.empty()) {
                break;
            } else {
                for (size_t i = 0; i < border_ind.size(); ++i) {
                    zones[border_ind[i]] = zone;
                }
                ++zone;
                std::vector<size_t> tmp;
                for (size_t i = 0; i < order_indices.size(); ++i) {
                    if (zones[order_indices[i]] == -1) {
                        tmp.push_back(order_indices[i]);
                    }
                }
                order_indices = tmp;
            }
        ...
        if (zone == 0) {
            for (size_t i = 0; i < order_indices.size(); ++i) {
                zones[order_indices[i]] = 0;
            }
        } else {
            for (size_t i = 0; i < order_indices.size(); ++i) {
                zones[order_indices[i]] = zone - 1;
            }
        }
        return zones;
    }
