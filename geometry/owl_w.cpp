...
namespace geometry {
    ...
    public:
        ...
        long double ArcLength(const Point& lhs, const Point& rhs) const;
        long double ArcLength(long double angle) const;
    };

    // [0; pi]
    long double Angle(const Vector& first, const Vector& second);
}

long double ShortestWay(const geometry::Point& begin,
                        const geometry::Point& end,
                        const geometry::Circle& prohibited) {
    geometry::Segment way(begin, end);
    geometry::Vector dir(begin, end);
    if (!prohibited.CrossSegment(way)) {
        return dir.Length();
    }

    geometry::Vector to_center(begin, prohibited.GetCenter());
    geometry::Vector from_center(prohibited.GetCenter(), end);

    long double before_circle = sqrt(pow(to_center.Length(), 2) - pow(prohibited.GetRadius(), 2));
    long double after_circle = sqrt(pow(from_center.Length(), 2) - pow(prohibited.GetRadius(), 2));
    long double angle = geometry::Pi - geometry::Angle(dir, to_center) - geometry::Angle(dir, from_center);
    angle -= acos(prohibited.GetRadius() / to_center.Length());
    angle -= acos(prohibited.GetRadius() / from_center.Length());
    return before_circle + after_circle + prohibited.ArcLength(angle);
}


int main() {
    long long x1, y1, x2, y2, x3, y3, radius;
    std::cin >> x1 >> y1 >> x2 >> y2 >> x3 >> y3 >> radius;
    geometry::Point begin(x1, y1);
    geometry::Point end(x2, y2);
    geometry::Circle hole(geometry::Point(x3, y3), radius);

    std::cout.precision(9);
    std::cout << std::fixed;
    std::cout << ShortestWay(begin, end, hole) << '\n';
...
namespace geometry {
    ...
    long double Circle::ArcLength(const Point& lhs, const Point& rhs) const {
        Vector a(center_, lhs), b(center_, rhs);
        long double c = Scalar(a , b) / a.Length() ;
        c /= b.Length();
        long double angle = acos(c);
        return angle * radius_;
    }

    long double Circle::ArcLength(long double angle) const {
        return angle * radius_;
    }

    long double Angle(const Vector& first, const Vector& second) {
        return acos(Scalar(first, second) / first.Length() / second.Length());
    }
}
