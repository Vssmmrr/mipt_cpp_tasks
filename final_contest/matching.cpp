...
int main() {
	...
	size_t num_vertex = first_size + second_size + 2;

	size_t start = 0;
	size_t end = num_vertex - 1;

	Graph list_graph(num_vertex);
	for (size_t from = 1; from <= first_size; ++from) {
		vertex_t to;
		std::cin >> to;
		while (to != 0) {
			int weight = 1;
			to += first_size;
			list_graph[from].push_back({ from , to , weight });
			list_graph[to].push_back({ to , from , 0 });

			list_graph[from].back().back_index = list_graph[to].size() - 1;
			list_graph[to].back().back_index = list_graph[from].size() - 1;

			std::cin >> to;
		}
		list_graph[from].push_back({ from , to , 0 });
		list_graph[to].push_back({ to, from , 1 });

		list_graph[from].back().back_index = list_graph[to].size() - 1;
		list_graph[to].back().back_index = list_graph[from].size() - 1;
	}

	for (size_t from = first_size + 1; from <= first_size + second_size; ++from) {
		size_t to = end;

		list_graph[from].push_back({ from, to, 1 });
		list_graph[to].push_back({ to, from, 0 });
	...
	auto flow = MaxFlow(list_graph, start, end);
	std::cout << flow << "\n";

	for (size_t from = 1; from <= first_size; ++from) {
		for (auto& edge : list_graph[from]) {
			if (edge.flow > 0) {
				std::cout << from << " " << edge.to - first_size << "\n";
			}
		}
	}
